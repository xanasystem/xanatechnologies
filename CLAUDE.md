...
- 301s from every URL of the old WordPress sitemap and the SEO sheet `XT26_XanaTechnologies_Redirecciones_301` (product pages go to `xanasystem.com/pim` and `/catalogos`) in `astro.config.mjs`; `scripts/fix-redirect-slashes.mjs` runs after `astro build` so `/old-url/` (trailing slash) also matches. Vercel builds with `npm run build` (`vercel.json`).
